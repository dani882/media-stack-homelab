...
def read_radarr_api_key(config_file: Path) -> str:
...
def torrent_is_safe_to_remove(
    ...
    if category != "movies":
        return False, (
            f"category is {category!r}, not 'movies'"
        )
...
def main() -> int:
    ...
    for item in queue.get(
        ...
        #
        # Then remove Radarr's stale queue entry.
        # The torrent is already gone, so Radarr must not try
